...
qq <- qq_plot(GAM_3, point_alpha = 0.1, method = "simulate") & theme_bw(8)
resid <- residuals_linpred_plot(GAM_3, point_alpha = 0.1)& theme_bw(8)
resid_hist <- residuals_hist_plot(GAM_3, n_bins = 50)& theme_bw(8)
appraise <- qq + resid + resid_hist + plot_layout(ncol = 1, nrow = 3)
ggsave("../Figures/Spring2024Revision/GAM3_Diagnostic.png", appraise,
...
seven <- draw(GAM_3, residuals = T, select = 6, grouped_by = T)+
  geom_line(size=0.95)+
  theme_bw(8)+
  ggtitle("")+
  labs(x = "Depth (m)", caption = "")+
  theme(axis.text.x = element_text(angle=45, hjust=1),
        plot.margin=unit(c(-0.50,0,0,0), "null"),
...
eleven <- draw(GAM_3, residuals = T, select = 10, grouped_by = T)+
  ...
  theme_bw(8)+
  ggtitle("")+
  labs(x = expression(Oxygen~(mg~L^-1)), caption = "")+
  theme(axis.text.x = element_text(angle=45, hjust=1),
        plot.margin=unit(c(-0.50,0,0,0), "null"),
        axis.title.x = element_text(size=8),
        legend.position = "none")+
...
fifteen <- draw(GAM_3, residuals = T, select = 14, grouped_by = T)+
  theme_bw(8)+
  ggtitle("")+
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.title.x = element_text(size=6),
...
ggsave("../Figures/Spring2024Revision/GAM3_fixed_env_Partials.png",env, units = "cm", width = 17, height = 15, dpi = 600)
