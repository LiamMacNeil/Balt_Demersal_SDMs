...
ggsave("../Figures/RF/Q4/CodAdult_Q4_2020_PredCor.png", pred_cor, width = 16, height = 12, units = "cm", dpi = 300)
...
write.csv(metrics, "../Model_Metrics/RF/CodAdult_Q4_2020_RF.csv")
...
RF_P <- predict(Q4_layers_balt[[varselect]], mod_RF_final, type = "response", ntree = which.min(mod_RF_final$mse))

# Nice collage plot from ?partialPlot
imp <- randomForest::importance(mod_RF_final)
impvar <- rownames(imp)[order(imp[, 1], decreasing=TRUE)]

png("../Figures/RF/Q4/CodAdult_Q4_2020_pdp.png",  width = 15, height = 12, units = "cm", res = 300)
...
write.csv(data.frame(data.frame(imp)), 
          "../Data/BITS_Var_RelInf/RF/CodAdult_Q4_2020_RelInf.csv")

saveRDS(mod_RF_final, file = paste0("../Models/RF/CodAdult_Q4_2020_Prediction.rds"))
...
#RF_P <- raster("../Predictions/Dab_Q4_2001_RF.tif")

png("../Figures/RF/Q4/CodAdult_Q4_2020_predmap.png", width = 15, height = 12, res = 300, units = "cm")
plot(abs(RF_P),
     col = cividis(20), 
...
writeRaster(RF_P, "../Predictions/CodAdult_Q4_2020_RF.tif", overwrite=TRUE)
