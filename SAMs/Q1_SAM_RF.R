...
#png("../Figures/RF/Spatial_autocorrelation/JuvenileCod_Q1_2020_blocking.png", width = 20, height = 12, res = 300, units = "cm")
sac1 <- cv_spatial_autocor(r = Q1_layers_balt[[varselect]], 
                           num_sample = 5000)
#dev.off()

#png("../Figures/RF/Spatial_autocorrelation/JuvenileCod_Q1_2020_variogram.png", width = 16, height = 10, res = 300, units = "cm")
...
png("../Figures/RF/Spatial_autocorrelation/JuvenileCod_Q1_2020_MESS.png", width = 16, height = 10, res = 300, units = "cm")
...
#png("../Figures/RF/Spatial_autocorrelation/JuvenileCod_Q1_2020_cv.png", width = 18, height = 12, res = 300, units = "cm")
cvp <- cv_plot(cv = bloCV, 
               x = sf,
               points_alpha = 0.5,
               label_size = 3)

ggsave("../Figures/RF/Spatial_autocorrelation/JuvenileCod_Q1_2020_cv.png", cvp, width = 24, height = 14, dpi = 300, units = "cm")
...
# RandomForests
####################### ####################### ####################### ####################### 
formula_RF <- as.formula(paste(names(full_bCV)[12], "~", paste(varselect, collapse = "+")))
formula_RF
...
for (i in folds){
  ...
  mtry_tune <- tuneRF(x = dat_train_fold[,c(1:9)],
                      y = dat_train_fold$Density_log10,
                      ntreeTry   = 500,
                      mtryStart  = 5,
                      stepFactor = 1.5,
                      improve    = 0.01)
  
  mod_RF <- randomForest(formula = formula_RF, 
                         data = dat_train_fold, 
  ...
  pred <- predict(mod_RF, dat_test_fold)
...
ggsave("../Figures/RF/Q1/JuvenileCod_Q1_2020_PredCor.png", pred_cor, width = 16, height = 12, units = "cm", dpi = 300)
...
write.csv(metrics, "../Model_Metrics/RF/JuvenileCod_Q1_2020_RF.csv")


hist(treesize(mod_RF),                        # give us the number of trees in term of number of nodes
...
############ Prediction ##################
mtry_tune <- tuneRF(x = full[varselect],
                    y = full$Density_log10,
                    ntreeTry   = 500,
                    mtryStart  = 5,
                    stepFactor = 1.5,
                    improve    = 0.01)

mod_RF_final <- randomForest(formula = formula_RF, 
                       data = full, 
                       na.action = na.exclude,
                       importance = T, 
                       proximity = T,
...
RF_P <- predict(Q1_layers_balt[[varselect]], mod_RF_final, type = "response", ntree = which.min(mod_RF_final$mse))

# Nice collage plot from ?partialPlot
imp <- randomForest::importance(mod_RF_final)
impvar <- rownames(imp)[order(imp[, 1], decreasing=TRUE)]

png("../Figures/RF/Q1/JuvenileCod_Q1_2020_pdp.png",  width = 15, height = 12, units = "cm", res = 300)
par(mfrow=c(3, 3), mai = c(0.35, 0.35, 0.35, 0.35))
for (i in seq_along(impvar)) {
  partialPlot(mod_RF, dat_train_fold, impvar[i], xlab=impvar[i],
              main=paste(impvar[i]),
              ylim=c(0, 1.5))
}
dev.off()

write.csv(data.frame(data.frame(imp)), 
          "../Data/BITS_Var_RelInf/RF/JuvenileCod_Q1_2020_RelInf.csv")

saveRDS(mod_RF_final, file = paste0("../Models/RF/JuvenileCod_Q1_2020_Prediction.rds"))

RF_P<- crop(mask(RF_P, icesarea), extent(icesarea)) 

# Back-transformation (log10) to kg km^-2
RF_P <- (10^(RF_P)-1)/1000


RF_P <- raster("../Predictions/Dab_Q1_2020_RF.tif")

png("../Figures/RF/Q1/Dab_Q1_2020_predmap.png", width = 15, height = 12, res = 300, units = "cm")
plot(abs(RF_P),
     col = cividis(20), 
...
raster::contour(abs((RF_P) > quantile(abs(RF_P), probs = 0.95)), lwd = 0.25, col="springgreen2", drawlabels = F, add=T)
#points(dat_test %>% filter(Density_log10 >0) %>% dplyr::select(Longitude, Latitude) , pch=21, bg="purple")

dev.off()

writeRaster(RF_P, "../Predictions/JuvenileCod_Q1_2020_RF.tif")


### Hyper grid method for tuning "ranger" randomForests
# hyperparameter grid search
hyper_grid <- expand.grid(
  mtry       = seq(1, 8, by = 2),
  node_size  = seq(2, 10, by = 2),
  sampe_size = c(.5, .65, .80, .95),
  OOB_RMSE   = 0
)
...
for(i in 1:nrow(hyper_grid)) {
  ...
  hyper_grid$OOB_RMSE[i] <- sqrt(model$prediction.error) #change correct
}

hyper_grid <- head(hyper_grid %>% arrange(min(OOB_RMSE)))

mod_RF <- ranger(formula = formula_RF,
                 data = dat_train,
                 num.trees = 1000,
