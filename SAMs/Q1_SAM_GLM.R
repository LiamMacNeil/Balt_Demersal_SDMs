...
#png("../Figures/GLM/Spatial_autocorrelation/CodAdult_Q1_2020_blocking.png", width = 20, height = 12, res = 300, units = "cm")
sac1 <- cv_spatial_autocor(r = Q1_layers_balt[[varselect]], 
                           num_sample = 5000)
#dev.off()

#png("../Figures/GLM/Spatial_autocorrelation/CodAdult_Q1_2020_variogram.png", width = 16, height = 10, res = 300, units = "cm")
...
#png("../Figures/GLM/Spatial_autocorrelation/CodAdult_Q1_2020_blocks.png", width = 16, height = 10, res = 300, units = "cm")
...
#png("../Figures/GLM/Spatial_autocorrelation/CodAdult_Q1_2020_MESS.png", width = 16, height = 10, res = 300, units = "cm")
...
#png("../Figures/GLM/Spatial_autocorrelation/CodAdult_Q1_2020_cv.png", width = 18, height = 12, res = 300, units = "cm")
cvp <- cv_plot(cv = bloCV, 
        x = sf,
        points_alpha = 0.5,
        label_size = 3)

#ggsave("../Figures/GLM/Spatial_autocorrelation/CodAdult_Q1_2020_cv.png", cvp, width = 24, height = 14, dpi = 300, units = "cm")
...
formula_GLM <- as.formula(paste(names(full_bCV)[12], "~", paste(varselect, collapse = "+")))
formula_GLM
...
for (i in folds){
  ...
  mod_GLM <- glm(formula = formula_GLM, family = gaussian(link = "identity"), data = dat_train_fold)
  
  print(summary(mod_GLM))
  
  mod_GLM = step(mod_GLM) 
  
  pred <- predict(mod_GLM, dat_test_fold)
  
  # MAE (accuracy)
  #print(mean(abs(dat_test_fold$Density_log10 - pred)))
  mae <- append(mae, list(mean(abs(dat_test_fold$Density_log10 - pred))))
  
  preds <- data_frame(dat_test_fold$folds,dat_test_fold$Density_log10, pred)
...
ggsave("../Figures/GLM/Q1/CodAdult_Q1_2020_PredCor.png", pred_cor, width = 16, height = 12, units = "cm", dpi = 300)
...
write.csv(metrics, "../Model_Metrics/GLM/CodAdult_Q1_2020_GLM.csv")
...
#ggsave("../Figures/GLM/Q1/CodAdult_Q1_2020_PredDens.png", pred_dens, width = 15, height = 12, units = "cm", dpi = 300)
...
GLM_final_mod <- step(glm(formula = formula_GLM, family = gaussian(link = "identity"), data = full))

# MODEL RESIDUALS
#par(mfrow = c(2,1))

# Randomly distributed with fitted values?
plot(resid(GLM_final_mod) ~ fitted(GLM_final_mod), xlab = 'Fitted Values', ylab = 'Residuals')

# Normally distributed?
qqnorm(resid(GLM_final_mod), main="normal qq-plot, residuals")
qqline(resid(GLM_final_mod))

#saveRDS(mod_GLM, file = paste0("../Models/GLM/CodAdult_Q1_2020_Valid.rds"))
saveRDS(GLM_final_mod, file = paste0("../Models/GLM/CodAdult_Q1_2020_Prediction.rds"))

par(mfrow=c(3, 3), mai = c(0.35, 0.35, 0.35, 0.35))

p <- plot_model(GLM_final_mod, type = "pred", title  = "")
nCol <- floor(sqrt((length(p))))

png("../Figures/GLM/Q1/CodAdult_Q1_2020_pdp.png", width = 15, height = 12, res = 300, units = "cm")
do.call("grid.arrange", c(p, ncol=nCol))
dev.off()

GLM_P <- predict(Q1_layers_balt[[varselect]], GLM_final_mod, type = "response")
GLM_P<- crop(mask(GLM_P, icesarea), extent(icesarea)) 

# Back-transformation (log10) to kg km^-2
GLM_P <- (10^(GLM_P)-1)/1000

GLM_P <- raster("../Predictions/CodAdult_Q1_2020_GLM.tif")

png("../Figures/GLM/Q1/CodAdult_Q1_2020_predmap.png", width = 15, height = 12, res = 300, units = "cm")
plot(abs(GLM_P),
     col = viridis(20), 
...
raster::contour(abs((GLM_P) > quantile(abs(GLM_P), probs = 0.95)), lwd = 0.25, col="firebrick2", drawlabels = F, add=T)
#points(dat_test %>% filter(Density_log10 >0) %>% dplyr::select(Longitude, Latitude) , pch=21, bg="purple")

dev.off()


writeRaster(GLM_P, "../Predictions/CodAdult_Q1_2020_GLM.tif", overwrite=TRUE)
