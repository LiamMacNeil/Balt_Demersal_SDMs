...
#png("../Figures/GAM/Spatial_autocorrelation/CodAdult_Q4_2020_blocking.png", width = 20, height = 12, res = 300, units = "cm")
sac1 <- cv_spatial_autocor(r = Q4_layers_balt[[varselect]], 
                           num_sample = 5000)
#dev.off()

#png("../Figures/GAM/Spatial_autocorrelation/CodAdult_Q4_2020_variogram.png", width = 16, height = 10, res = 300, units = "cm")
...
#png("../Figures/GAM/Spatial_autocorrelation/CodAdult_Q4_2020_blocks.png", width = 16, height = 10, res = 300, units = "cm")
...
#png("../Figures/GAM/Spatial_autocorrelation/CodAdult_Q4_2020_MESS.png", width = 16, height = 10, res = 300, units = "cm")
...
#png("../Figures/GAM/Spatial_autocorrelation/CodAdult_Q4_2020_cv.png", width = 18, height = 12, res = 300, units = "cm")
cvp <- cv_plot(cv = bloCV, 
               x = sf,
               points_alpha = 0.5,
               label_size = 3)

#ggsave("../Figures/GAM/Spatial_autocorrelation/CodAdult_Q4_2020_cv.png", cvp, width = 24, height = 14, dpi = 300, units = "cm")
...
gam1 <- mgcv::gam(Density_log10 ~ s(Bottom.salinity)+s(Bottom.temperature)+s(Bottom.oxygen)+s(Chlorophyll)+s(MLD)+s(Surface.temperature), 
                  data = full, family = gaussian, method = 'ML', select = T)
gam2 <- mgcv::gam(Density_kg ~ s(Bottom.salinity)+s(Bottom.temperature)+s(Bottom.oxygen)+s(Chlorophyll)+s(MLD)+s(Surface.temperature), 
...
for (i in folds){
  ...
  mod_GAM <- mgcv::gam(Density_log10 ~ s(Bottom.salinity)+s(Bottom.temperature)+s(Bottom.oxygen)+s(Chlorophyll)+s(MLD)+s(Surface.temperature), 
                       data = dat_train_fold, family = gaussian, method = 'ML', select = T, gamma = 1.4)
...
ggsave("../Figures/GAM/Q4/CodAdult_Q4_2020_PredCor.png", pred_cor, width = 16, height = 12, units = "cm", dpi = 300)
...
write.csv(metrics, "../Model_Metrics/GAM/CodAdult_Q4_2020_GAM.csv")
...
#ggsave("../Figures/GAM/Q4/CodAdult_Q4_2020_PredDens.png", pred_dens, width = 15, height = 12, units = "cm", dpi = 300)
...
GAM_final_mod <- mgcv::gam(Density_log10 ~ s(Bottom.salinity)+s(Bottom.temperature)+s(Bottom.oxygen)+s(Chlorophyll)+s(MLD)+s(Surface.temperature), 
                           data = full, family = gaussian, method = 'ML', select =T, gamma = 1.4)


#saveRDS(mod_GAM, file = paste0("../Models/GAM/CodAdult_Q4_2020_Valid.rds"))
saveRDS(GAM_final_mod, file = paste0("../Models/GAM/CodAdult_Q4_2020_Prediction.rds"))
...
png("../Figures/GAM/Q4/CodAdult_Q4_2020_pdp.png", width = 15, height = 12, res = 300, units = "cm")
do.call("grid.arrange", c(p, ncol=nCol))
dev.off()

GAM_P <- predict(Q4_layers_balt[[varselect]], GAM_final_mod, type = "response")
GAM_P<- crop(mask(GAM_P, icesarea), extent(icesarea)) 

# Back-transformation (log10) to kg km^-2
GAM_P <- (10^(GAM_P)-1)/1000

GAM_P <- raster("../Predictions/CodAdult_Q4_2020_GAM.tif")

png("../Figures/GAM/Q4/CodAdult_Q4_2020_predmap.png", width = 15, height = 12, res = 300, units = "cm")
plot(abs(GAM_P),
     col = viridis(20), 
...
writeRaster(GAM_P, "../Predictions/CodAdult_Q4_2020_GAM.tif", overwrite=TRUE)
